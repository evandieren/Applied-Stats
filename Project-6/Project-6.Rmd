...
In this project, we focus on the first wave in European countries, and more precisely on the curve of covid cases across those countries. From those curves, we try to common patterns around the mean. Practically, this can be done using Functional Principal Component Analysis, where principal components are functions and each curve will be a linear combination of those functions plus the mean from the selected countries covid cases..
...
Practically, we set the order of B-Splines to six, and set $\{z_i\}_{i=1,...,M}$ as the weekly nodes instead of the original daily nodes as it created stability issues for the smoothing when reaching $t=1$.
...
The first row of the plots below represent the mean of the log-cumulative total cases for $0 \le t \le 1$ (left), and the scores of each country with respect to the first two principal components. The next four plots represent the principal components, and in their title, we compute the percentage of total variance explained.
...
```
First, we observe on the top-left plot the red line which represent the mean of log-cumulative number of covid cases for the $24$ retained European countries for $0 \le t \le 1$. We observe a sort of square root increase and a stabilization around $t = 0.5$. Regarding the interpretation of our principal components, the first one shows a growth and stabilization at $t = 0.4$. The second one shows a first bump, following by a steep decrease up to the negative values. The third one also shows a rapid growth, followed by a steep decrease before starting to linearly increase again. The fourth PC shows the same initial bump, followed by an inverse bump and a slight decrease from $t=0.4$.

One notes that we left out the plot of PC scores from the short analysis above. If one gets the GDP of each retained country for $2019$, we can plot those PC scores and apply the GDP as a color to the datapoints on a scatter plot :
...
As seen above, we observe a strong concentration of the PC scores around the $0-0$ point. This means that the majority of the countries are not too far from the mean (as seen on the top-left plot from the previous figure). We observe two outliers from this plot. First, we note that Austria has a significant positive coefficient for the first PC. This means that it will be placed above the mean, as it takes the PC1 is positive and steeply increasing for small $t$. It also has a strongly negative coefficient for the second PC, meaning that its curve will first be below the mean as it will decrease, then start raising after $t=0.2$. The second outlier is Belgium for which both PC scores are negative.
