...
As one can see above, we see that coefficients are essentially close to zero, as the values we are trying to plot are usually close to zero, as seen in the histograms above. For the quadratic and exponential term, it is even essentially zero, with a high level of significance. The months coefficients have non-zero values but are less significant, except for March, June, September ($0.09$) and July ($0.06$) with p-values in parentheses.

However, one big dragdown using this linear model is that we assume that the data to be iid in order to use  significance tests, which is not the case here. Therefore, we can apply a trick by using Generalized Least Squares: 
...
To confirm what we have seen in the summary of the GLS model above, this ANOVA test confirms that the exponential variable is not significant for this model. *We can therefore refute the exponential trend* of the temperature anomalies. Furthermore, one can also argue that the quadratic trend has a significant zero coefficient, which also indicates that the trend might not be quadratic. Only the linear term has non-zero significant coefficient. We can therefore state that the temperature anomalies seem linear, starting from $1960$ onwards.

Additionally, we observe a strong significance regarding the `month` factor, meaning that *there is seasonality regarding the temperature month per month*. However, as noted in the introduction, this could be due to more land in the north hemisphere than in the south.

# Prediction up to $2050$

First, we can remove the quadratic and exponential trend from the original model, as it has been shown to be non-significant, our final model is therefore :

$$\tilde{Y} =  \beta_0 + \beta_1 t + \epsilon \quad \text{ with } \epsilon \sim ARMA(p=2,q=2)$$
```{r}
R_struct <- corARMA(form=~t, p=2,q=2) # ARMA(2,0,2) correlation structure 
sig_glsfit <- gls(y~t + month, data=lmData, corr=R_struct, method="ML")
```
...
As previously observed, we still obtain better results for the ARMA(p=2,q=2) model.
...
qqnorm(resid_f_fit, main = "Normal Q-Q Plot of the ARMA(p=2,q=2) residuals")
qqline(resid_f_fit)
```
