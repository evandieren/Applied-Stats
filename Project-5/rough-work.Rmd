...
# prove or refute the exponential trend : model linear + poly + exp and check params significance
# Utiliser stl() pour voir la décomp et voir la trend
```
...
## Finding a trend for the temperature time-series

```{r}
# Trying to remove the upward trend
...
## Fitting a SARIMA model on the residual
...
fit <- arima(ts_res,order=c(2,0,2))
...
```
Now, one can check if a submodel might do a better job

```{r}
fit_21 <- arima(ts_res, order=c(2,0,1))
fit_12 <- arima(ts_res, order=c(1,0,2))
fit_11 <- arima(ts_res, order=c(1,0,1))
cat(c(fit$aic,fit_21$aic,fit_12$aic, fit_11$aic))
...
# fit 202 , fit 201, fit 102
#0.03298126 0.03331282 0.03331522 -> fit 202 is better
```

## Fitting the gls model with ARMA covariance

```{r}
R_struct <- corARMA(form=~t, p=2,q=2) # AR(2,0,2) correlation structure 
...
```
So basically, it is the same as with the exponential model we found earlier. Now, lets test them


## Testing for significance (answering both questions)
...
anova(glsfit,sig_glsfit)
```

From this test, we observe the non-significance of the exponential part of the modeling, and the significance of the linear and quadratic dependance on time. This helps us answering the question regarding the exponential growth of the temperature around the globe : *it is not significant*, one could see that a low quadratic function might work better to fit the model. Furthermore, we observe a strong significance regarding the `month` factor, meaning that there is seasonality regarding the temperature month per month, even though the collected data should be without seasonality as it is supposed to represent the global temperature around the world. This could be due to more measurements made in the north or south hemisphere, which could bias the total mean to a north or south hemisphere seasonal effect.

## Predicting temperature until 2050

```{r}

start <- length(ts_temp)+1
end <- start+12*50-1

pred_data <- data.frame(month=as.factor(rep(1:12,50)),
                     t = start:end)
...
plot(ts_pred)
PlotACF(resid(sig_glsfit))
```


Questions for Prof :

1) Once I have this gls model, do I still have to redo everything regarding the noise? Seems like yes, but then why did I have to give it a ARMA strucutre?

2) Is it the good methodology? Can I conclude strictly from those anova tests that there is a seasonality and no exponential rate?

3) By adding the months, don't we ourselves add the seasonality?

4) Is the method for GAM correct ? If so, do I do the same by applying GLS ? (refers to 1))
...
# Using GAMs and SARIMA model for residual (Prediction only)

Instead of working with Generalized Least Square, one can first try to fit the trend with a Generalized Additive model and then find SARIMA parameters for its residuals. This can help us answer the seasonality question based on the parameters found in the SARIMA model for residuals, but does not answer the question regarding the exponential growth.
