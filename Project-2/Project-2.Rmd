...
The next step is to fit the model containing only the statistically significant covariates, which we will call $glm_{sig}$. With this model, we obtain an AUC of $0.8878$, an AIC of $7145$ and a BIC of $7344$. All those metrics are slightly better than $glm_0$. Furthermore, with $H_0$ being that the predictors not contained in the sub-model do not influence the outcome, one can check this hypothesis using the likelihood ratio test (model-submodel test). We get a p-value of $0.35$, which means that $H_0$ can not be rejected and therefore shows that the sub-model is significant.
...
glm_enhanced <- glm(purchase ~ n_product_page + exit_rates + page_values + month
                    ...
                    data=Data,family = "binomial")
#AUC_eval(glm_enhanced,Data) # 0.9043155 vs AUC_eval(glm_sig,Data)) 0.8878329
#AIC(glm_enhanced) # 6514.713 vs AIC(glm_sig) # 7145.045
#BIC(glm_enhanced) # 6735.554 vs BIC(glm_sig) # 7343.802
```
With this enhanced model $glm_{en}$, we get an AUC of 0.9043 which is beating the benchmark. Additionally, it has a AIC of $6515$, a BIC of $6735$, which show a strong improvement from the previous model by adding nonlinearities.
...
Anova(glm_final,type=2,test="LR") %>% 
  kable(digits = 3,caption = "ANOVA Test-II Table with final model") %>% 
  kable_styling()
...
As shown above, we get the final model by adding every aspect of the previously discussed points, such as the significant initial covariates, the non-linear dependences regarding `page_values`, `n_product_page` and `n_admin_page` as well as two interactions. This model was significantly better than the previous iterations as seen above and arrive to a $0.905$ AUC which beats the benchmark.
...
summ(glm_final,confint = TRUE,digit=3)
```
