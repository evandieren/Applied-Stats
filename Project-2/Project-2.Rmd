...
#Anova(glm_sig,type=2,test="LR") # everything is significant (as expected)
cat(AUC_eval(glm_sig,Data)) # 0.8878329
AIC(glm_sig) # 7145.045
BIC(glm_sig) # 7343.802

anova(glm0,glm_sig,test="LRT")
```
The next step is to fit the model containing only the statistically significant covariates, which we will call $glm_{sig}$. With this model, we obtain an AUC of $0.8878$, an AIC of $7145$ and a BIC of $7344$. All those metrics are slightly better than $glm_0$, even though the model-submodel test using the Loglikelihood ratio test has a p-value of $0.35$.
